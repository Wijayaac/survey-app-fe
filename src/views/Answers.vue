		...
		<div v-else-if="Object.keys(surveys).length">
			<div class="grid grid-cols-1 h-full md:grid-cols-3">
				<div class="col-span-1 h-full">
					<ul>
						<li v-for="survey in surveys" :key="survey.id" @click="selectSurvey(survey.id)"
							...
							<div class="flex-1">
								...
								<p class="text-indigo-500">{{ survey.status ? 'Active' : 'Ended' }}</p>
				...
				<div class="col-span-2 ml-4 hidden md:block">
					...
					<div v-else class="p-4 shadow-md rounded-b-sm bg-white animate-fade-in-down" style="animation-delay: 0.2s">
						...
						<p class="text-sm font-medium mb-2 text-emerald-500">{{ selectedSurvey.status ? 'Active' : 'Finished' }} -
							{{ selectedSurvey.questions.length }} Questions</p>
						<ul>
...
import {ClockIcon, EyeIcon, ChevronRightIcon, FolderIcon} from "@heroicons/vue/24/outline/index.js";
