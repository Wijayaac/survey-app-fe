  ...
  <PageComponent title="Survey Answers">
    ...
    <div v-else class="p-8 bg-white shadow-md rounded-md">
      ...
      <p class="text-emerald-500 text-sm mb-2">
        {{ survey.status ? "Active" : "Finished" }}
      </p>
      <ul class="mb-4">
        <li class="flex items-center mb-2">
          <QuestionMarkCircleIcon
            class="btn-icon text-gray-900 text-indigo-500"
          />
          ...
          <table class="border-collapse table-fixed w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
              <tr v-for="answer in answers" :key="answers.id">
                ...
                <td
                  class="border-b border-slate-100 dark:border-slate-700 p-4 text-gray-700 dark:text-gray-700"
                >
                  {{ answer.created_at }}
                </td>
                <td
                  class="border-b border-slate-100 dark:border-slate-700 p-4 pr-8 text-gray-700 dark:text-gray-700"
                >
                  {{ answer.expire_date }}
                </td>
                <td
                ...
                >
                  <router-link
                    :to="{
                      name: 'answer-detail',
                      params: { slug: survey.slug, id: answer.id },
                    }"
                  >
                    <EyeIcon
                      class="btn-icon transition-colors hover:text-indigo-400"
                    />
                  </router-link>
                </td>
              </tr>
...
async function getForPage(link) {
  ...
  await store.dispatch("getAnswers", {
    url: `${link.url}&id=${route.params.id}`,
  });
}

async function exportData() {
  let response = await axiosClient.get(`/export?survey=${route.params.id}`, {
    responseType: "blob",
  });
