		...
		<Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
						...
						<div class="hidden md:block">
							<div class="ml-10 flex items-baseline space-x-4">
								<router-link v-for="item in navigation" :key="item.name" :to="item.to"
														 ...
														 :aria-current="this.$route.name === item.to.name ? 'page' : undefined">{{ item.name }}
								</router-link>
							</div>
						</div>
					</div>
					<div class="hidden md:block">
						<div class="ml-4 flex items-center md:ml-6">
							...
							<Menu as="div" class="relative ml-3">
								<div>
									<MenuButton
											class="max-w-xs rounded-full !p-1 bg-gray-800 !focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
										<span class="sr-only">Open user menu</span>
										<UserCircleIcon class="btn-icon !m-0 p-0"/>
									</MenuButton>
								</div>
								<transition enter-active-class="transition ease-out duration-100"
									...
									<MenuItems
										...
										<MenuItem v-slot="{ active }">
											<a @click="logout" class="cursor-pointer block px-4 py-2 text-sm text-gray-700">Sign Out</a>
										</MenuItem>
									</MenuItems>
								</transition>
							</Menu>
			...
			<DisclosurePanel class="md:hidden">
				<div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
					<router-link v-for="item in navigation" :key="item.name" active-class="bg-gray-900 text-white" :to="item.to"
				...
				<div class="border-t border-gray-700 pt-4 pb-3">
					<div class="flex items-center px-5">
						<div class="flex-shrink-0">
							<UserCircleIcon class="btn-icon text-white"/>
						</div>
						<div class="ml-3">
